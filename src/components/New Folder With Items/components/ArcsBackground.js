...
const ArcsBackground = ({context,thickness,innerRadius,width,height,arcIndex,hoveredIndex,setHoveredIndex, avgDeg, redDeg}) => {
  ...
  const startXOrbit = Math.cos((avgDeg < redDeg ? avgDeg : redDeg) * radians) * (outerRadius - innerRadius + 5);
  const startYOrbit = Math.sin((avgDeg < redDeg ? avgDeg : redDeg) * radians) * (outerRadius - innerRadius + 5);

  const endXOrbit = Math.cos((avgDeg > redDeg ? avgDeg : redDeg) * radians) * (outerRadius - innerRadius - 5);
  const endYOrbit = Math.sin((avgDeg > redDeg ? avgDeg : redDeg) * radians) * (outerRadius - innerRadius - 5);

  const orbitPathGen = path();

  orbitPathGen.moveTo(startXOrbit, startYOrbit);
  orbitPathGen.arc(0, 0, (innerRadius + thickness / 2 + 2), (avgDeg < redDeg ? avgDeg : redDeg) * radians, (avgDeg > redDeg ? avgDeg : redDeg) * radians);
  orbitPathGen.lineTo(endXOrbit, endYOrbit);
  orbitPathGen.arc(0, 0, (innerRadius + thickness / 2 - 2), (avgDeg > redDeg ? avgDeg : redDeg) * radians, (avgDeg < redDeg ? avgDeg : redDeg) * radians, true);
  ...
  for (let i = 0; i < numSegments; i++) {
    ...
    textPathGen.arc(width/2, height, 215, segmentAngles[i] * (Math.PI / 180), segmentAngles[i+1]*(Math.PI / 180));
        ...
        if (index >= 0){
          ...
          return (
            ...
            <textPath xlinkHref={`#arc-${index}`} startOffset="50%">
              <tspan x={0} dy={index === 3 ? 0 : "-1.2em"}>{['Extremely Low', 'Significantly Low', 'Somewhat Low', 'Average', 'Somewhat Elevated','Significantly Elevated', 'Extremely Elevated'][index].split(' ')[0]}</tspan>
                <tspan x={0} dy={index === 3 ? 0 : "1.2em"}>{['Extremely Low', 'Significantly Low', 'Somewhat Low', 'Average', 'Somewhat Elevated','Significantly Elevated', 'Extremely Elevated'][index].split(' ')[1]}</tspan>
            </textPath>
          </text>
          <path id={`arc-bg-${index}`} d={textPaths[index]} style={{fill:'none'}}></path>
          </g>
          )
        }
        })}
