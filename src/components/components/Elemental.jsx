...
const Elemental = ({
  ...
  const [stBaseRects, setBaseRects] = useState([]);
  const [stBaseArcs, setBaseArcs] = useState([]);
  ...
  useEffect(() => {
    ...
    const basePathGenerator = path();

    const startBAngle = -180;
    const endBAngle = 0;

    const startBXOuter = Math.cos(startBAngle * radians) * outerRadius;
    const startBYOuter = Math.sin(startBAngle * radians) * outerRadius;

    const endBXInner = Math.cos(endBAngle * radians) * innerRadius;
    const endBYInner = Math.sin(endBAngle * radians) * innerRadius;

    const endBXOuter = Math.cos(endBAngle * radians) * outerRadius;
    const endBYOuter = Math.sin(endBAngle * radians) * outerRadius;

    basePathGenerator.moveTo(startBXOuter, startBYOuter);

    basePathGenerator.arc(
      0,
      0,
      outerRadius,
      startBAngle * radians,
    ...
    basePathGenerator.lineTo(endBXInner, endBYInner);
    basePathGenerator.arc(
      0,
      0,
      innerRadius,
    ...
    basePathGenerator.closePath();

    baseArcs.push(basePathGenerator.toString());
    setBaseArcs(baseArcs);

    const baseRects = [];

    const rectBasePathGenerator = path();
    rectBasePathGenerator.moveTo(-outerRadius, 0);
    rectBasePathGenerator.lineTo(-outerRadius, -thickness);
    rectBasePathGenerator.lineTo(outerRadius, -thickness);
    rectBasePathGenerator.lineTo(outerRadius, 0);
    rectBasePathGenerator.closePath();
    baseRects.push(rectBasePathGenerator.toString());
    console.log(baseRects);
    setBaseRects(baseRects);
    const rects = [];
    for (let i = 0; i < numSegments; i++) {
      if (i < 3) {
    ...
    <>
      {drawnArcs.map((arcPath, index) => {
        if (index > -1) {
          return (
            <>
              <mask id={"mask" + index} transform="translate(100,30)">
                <path
              ...
              >
                <path
                  d={isClicked ? stBaseArcs[0] : stBaseRects[0]}
                  transform={`translate(${width / 2},${height})`}
                  style={{ opacity: 1 }}
                />
              </g>
